...
void LCD_zoomChar(uint16_t in_width,	//原始字符宽度
									...
									const uint16_t *in_ptr,	//字库输入指针	注意：1pixel 1bit
									uint8_t *out_ptr)			//缩放后的字符输出指针 注意: 1pixel 8bit
{
	uint8_t *pts,*ots;
	unsigned int xrIntFloat_16=(in_width<<16)/out_width+1; //Src ???,Dst ????
  unsigned int yrIntFloat_16=(in_heig<<16)/out_heig+1;
	...
	for(y=0;y<in_heig;y++)	//字库的数据映射到8bit
	...
	out_ptr = (uint8_t*)&zoomBuff;	//out_ptr没有正确传出，后面调用改成了全局变量指针算了
	pts =out_ptr;//这句话不需要，只是为了消除warning
}									

void LCD_DrawChar_Ex(uint16_t Xpos, uint16_t Ypos,uint16_t Font_width,uint16_t Font_Heig,uint8_t *c,uint16_t DrawModel)
{
  uint32_t index = 0, counter = 0, xpos =0;
  uint32_t  Xaddress = 0;
														 ...
														 uint16_t DrawModel)  //是否反色显示
{
	...
	while ((refcolumn < LCD_PIXEL_WIDTH) && ((*ptr != 0) & (((refcolumn + LCD_Currentfonts->Width) & 0xFFFF) >= LCD_Currentfonts->Width)))
	{
		...
		Ascii = *ptr - 32;
		LCD_zoomChar(16,24,Charwidth,Font_Heig,&LCD_Currentfonts->table[Ascii * LCD_Currentfonts->Height],psr);
